 ...
 \usage{
SKAT(Z,y, X=NULL,kernel = c("linear.weighted","linear","IBS", \dots), 
  out_type=c("C","D"), method=c("davies","liu","liu.mod","optimal"), 
  weights.beta=c(1,25), weights=NULL, impute.method=c("random","fixed"),
  is_intercept=TRUE, r.corr=0, is_check_genotype=TRUE )

SKAT(Z, obj, kernel = c("linear.weighted","linear","IBS", \dots), 
  method=c("davies","liu","liu.mod", "optimal"), weights.beta=c(1,25), weights=NULL, 
  impute.method=c("random","fixed"), r.corr=0, is_check_genotype=TRUE )

SKAT.SSD.OneSet = function(SSD.INFO, SetID, y, X=NULL, \dots)

SKAT.SSD.OneSet = function(SSD.INFO, SetID, obj, \dots)


SKAT.SSD.OneSet_SetIndex = function(SSD.INFO, SetIndex, y, X=NULL, \dots)
...
\arguments{
      ...
      \item{y}{a numeric vector of the outcome variables.}
      \item{X}{a numeric matrix of covariates with each row as a different individual and each column as a separate covariate measurement. If there is no covariate, it does not need to be specified.}
      ...
      \item{impute.method}{a method to impute missing genotypes (default= "random"). "random" imputes missing genotypes by generating binomial(2,p) random variables (p is the MAF), and "fixed" imputes missing genotypes by assigning the mean genotype value (2p). If you use "random", you will have different p-values for different runs because imputed values are randomly assigned.}
      \item{is_intercept}{a logical value indicating whether the model has an intercept (default= TRUE). When it is FALSE, the model does not have an intercept, and covariates matrix should not be NULL. }
...
The p-value of ``davies'' is computed using the CompQuadForm package. 
...
r.corr represents the \eqn{\rho} parameter of an unified test, 
...
# or
SKAT(Z, y.c, NULL, kernel = "linear.weighted", out_type = "C")$p.value
SKAT(Z, y.b, NULL, kernel = "linear.weighted", out_type = "D")$p.value
